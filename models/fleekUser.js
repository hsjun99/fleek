...
const fleekUser = {
  ...
  updateSetting: async (uid, data) => {
    const query1 = `UPDATE ${table1} SET is_kilogram = ${data.is_kilogram}, is_meter = ${data.is_meter}, lang_code = ${data.lang_code} WHERE uid="${uid}"`;
    try {
      await pool.queryParamMaster(query1);
      return true;
    } catch (err) {
    ...
    try {
      ...
      if (squat1RM != null) {
        const { inclination, intercept } = await getWorkoutEquation(
          ...
          await weightGroupClassifier(weight)
        );
