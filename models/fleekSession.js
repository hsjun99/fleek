...
*/
  ...
  modifySessionData: async (
    ...
    const ts3 = async (connection) => {
      ...
      await asyncForEach(data, async (workouts) => {
        ...
        if (
          workouts.super_set_label == null ||
          workouts.super_set_label == undefined
        )
          workouts.super_set_label = 0;
        await asyncForEach(workouts.detail, async (sets) => {
          ...
          if (result9[0].record_type == 4) {
            ...
            max_volume = Math.max(
              max_volume,
              sets.reps * (sets.weight + body_weight)
            );
            ...
            max_volume = Math.max(
              max_volume,
              sets.reps * (body_weight - sets.weight)
            );
          } else {
            total_volume += sets.reps * sets.weight * result9[0].multiplier;
            max_volume = Math.max(
              max_volume,
              sets.reps * sets.weight * result9[0].multiplier
            );
          }
          max_one_rm = Math.max(
            max_one_rm,
            await oneRmCalculator(sets.weight, sets.reps)
          );
          ...
          if (
            ...
            max_speed = Math.max(
              max_speed,
              sets.distance / (sets.duration / 60)
            );
          }
        });
    ...
    try {
      ...
      const addWorkoutlog = async () => {
        await asyncForEach(data, async (workouts) => {
          ...
          // Update UserWorkoutHistory Table - finish_num
          const fields11 =
            "add_num, finish_num, userinfo_uid, workout_workout_id";
          const questions11 = `?, ?, ?, ?`;
          const values11 = [0, 0, uid, workouts.workout_id];
          const query11 = `INSERT IGNORE INTO ${table_userWorkoutHistory}(${fields11}) VALUES(${questions11})`;
          const query5 = `UPDATE ${table_userWorkoutHistory} SET finish_num = finish_num+1 WHERE userinfo_uid = "${uid}" AND workout_workout_id="${workouts.workout_id}"`;
  ...
  postBookSession: async (uid, template_id, set_time, is_alarm) => {
    const fields1 =
      "userinfo_uid, templateUsers_templateUsers_id, onesignal_id, set_time";
    const questions1 = "?, ?, ?, ?";
    ...
    try {
      ...
      await pool.queryParamArrMaster(query1, [
        uid,
        template_id,
        onesignal_id,
        set_time,
