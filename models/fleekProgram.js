...
const template = {
    ...
    const restructure = async (result) => {
      ...
      await asyncForEach(result, async (rowdata) => {
        ...
        if (currentStatus.length == 0) {
          data.push({
            alphaProgram_id: rowdata.alphaProgram_id,
            name: rowdata.name,
            weeks: rowdata.weeks,
            days: rowdata.days,
  ...
  checkProgramUserStatus: async (uid, program_id) => {
    ...
    try {
      ...
      if (result.length == 0) {
        ...
        // Not Initialized
        return 0;
      } else if (result[0].progress == 0) {
        // Progress 0
        return 1;
      } else {
        // Initialized and Over Progress 1
        return 2;
      }
    } catch (err) {
      if (err.errno == 1062) {
  ...
  getProgramData: async (uid, program_id) => {
    const fields1 =
      "alphaProgram_id, alphaProgramUsers_id, name, weeks, days, objective, target, progress, workouts_index, one_rms_index";
    const fields2 =
      "alphaProgramUsers_id, progress, workouts_index, one_rms_index, alphaProgram_alphaProgram_id";
    ...
    const restructure = async (programOutline, one_rms_index) => {
      ...
      await asyncForEach(programOutline, async (sessionPlan) => {
        data.push([]);
        await asyncForEach(sessionPlan, async (workoutPlan) => {
          await data[data.length - 1].push({
            workout_id: workoutPlan.workout_id,
            workouts_index: workoutPlan.workouts_index,
            detail_plan: [],
          });
          const { min_step } = await getWorkoutInfo(workoutPlan.workout_id);
          await asyncForEach(
            [...Array(workoutPlan.reps.length).keys()],
            async (index) => {
              ...
              await data[data.length - 1][
                ...
                reps: workoutPlan.reps[index],
                weight: roundNumber.roundNum(
                  one_rm * load + min_step * workoutPlan.minStepConstant[index],
                  min_step
                ),
                rest_time: workoutPlan.restTime[index],
              });
            }
          );
        });
    ...
    try {
      const result = await pool.queryParamMaster(query);
      const workouts_index = JSON.parse(
        result[0].workouts_index
      ).workouts_index;
  ...
  getProgramWorkoutsDataByIndex: async (
    uid,
    program_id,
    sex,
    ageGroup,
    ...
    try {
      ...
      const workouts_index_default = JSON.parse(
        result1[0].workouts_index_default
      ).workouts_index;
      let one_rms_index = [
      ...
      await Promise.all(
        workouts_index_default.map(async (elem, index) => {
          if (!elem == 0) {
  ...
  initializeProgram: async (
    uid,
    program_id,
    workouts_index_default,
    one_rms_index
  ) => {
    const fields =
      "progress, workouts_index, one_rms_index, is_done, alphaProgram_alphaProgram_id, userinfo_uid";
    const questions = "?, ?, ?, ?, ?, ?";
    const workouts_index_default_String = JSON.stringify({
      workouts_index: workouts_index_default,
    });
    const one_rms_index_String = JSON.stringify({
      one_rms_index: one_rms_index,
    });
    const values = [
      0,
      workouts_index_default_String,
      one_rms_index_String,
      0,
      program_id,
      uid,
    ];
