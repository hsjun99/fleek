...
const workout = {
        ...
        try {
            ...
            const restructure = async () => {
                ...
                await asyncForEach(result, async (rowdata) => {
                    ...
                    if (data.length == 0) {
                        data.push({ session_id: rowdata.session_session_id, template_id: rowdata.templateUsers_id, template_name: (rowdata.session_name != null ? rowdata.session_name : rowdata.template_name), session_detail: { created_at: rowdata.created_at, start_time: rowdata.start_time, total_workout_time: rowdata.total_time, content: [{ workout_id: rowdata.workout_workout_id, super_set_label: rowdata.super_set_label, max_heart_rate: rowdata.max_heart_rate, sets: [{ reps: rowdata.reps, weight: rowdata.weight, duration: rowdata.duration, distance: rowdata.distance, rpe: rowdata.rpe }], workout_ability: { max_one_rm: rowdata.max_one_rm, total_volume: rowdata.total_volume, max_volume: rowdata.max_volume, total_reps: rowdata.total_reps, max_weight: rowdata.max_weight, percentage: percentage } }] } });
                    }
                    else if (data[data.length - 1].session_id == rowdata.session_session_id) {
                        const L = data[data.length - 1].session_detail.content.length
                        if (data[data.length - 1].session_detail.content[L - 1].workout_id == rowdata.workout_workout_id) {
                            data[data.length - 1].session_detail.content[L - 1].sets.push({ reps: rowdata.reps, weight: rowdata.weight, duration: rowdata.duration, distance: rowdata.distance, rpe: rowdata.rpe });
                        }
                        else {
                            data[data.length - 1].session_detail.content.push({ workout_id: rowdata.workout_workout_id, super_set_label: rowdata.super_set_label, max_heart_rate: rowdata.max_heart_rate, sets: [{ reps: rowdata.reps, weight: rowdata.weight, duration: rowdata.duration, distance: rowdata.distance, rpe: rowdata.rpe }], workout_ability: { max_one_rm: rowdata.max_one_rm, total_volume: rowdata.total_volume, max_volume: rowdata.max_volume, total_reps: rowdata.total_reps, max_weight: rowdata.max_weight, percentage: percentage } });
                        }
                    }
                    else {
                        data.push({ session_id: rowdata.session_session_id, template_id: rowdata.templateUsers_id, template_name: (rowdata.session_name != null ? rowdata.session_name : rowdata.template_name), session_detail: { created_at: rowdata.created_at, start_time: rowdata.start_time, total_workout_time: rowdata.total_time, content: [{ workout_id: rowdata.workout_workout_id, super_set_label: rowdata.super_set_label, max_heart_rate: rowdata.max_heart_rate, sets: [{ reps: rowdata.reps, weight: rowdata.weight, duration: rowdata.duration, distance: rowdata.distance, rpe: rowdata.rpe }], workout_ability: { max_one_rm: rowdata.max_one_rm, total_volume: rowdata.total_volume, max_volume: rowdata.max_volume, total_reps: rowdata.total_reps, max_weight: rowdata.max_weight, percentage: percentage } }] } });
    ...
    getOneRmMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
    ...
    getWeightMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
        const condition_group_body = `WHERE userinfo.uid
                                        IN (SELECT uid FROM userinfo
                                        WHERE sex = ${sex} AND
                                        CASE WHEN sex=0
    ...
    getVolumeMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
        const condition_group_body = `WHERE userinfo.uid
                                        IN (SELECT uid FROM userinfo
                                        WHERE sex = ${sex} AND
                                        CASE WHEN sex=0
    ...
    getTotalSetsMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
        const condition_group_body = `WHERE userinfo.uid
                                        IN (SELECT uid FROM userinfo
                                        WHERE sex = ${sex} AND
                                        CASE WHEN sex=0
    ...
    getTotalVolumeMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
        const condition_group_body = `WHERE userinfo.uid
                                        IN (SELECT uid FROM userinfo
                                        WHERE sex = ${sex} AND
                                        CASE WHEN sex=0
    ...
    getTotalRepsMaxListByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition, page) => {
        const condition_group_body = `WHERE userinfo.uid
                                        IN (SELECT uid FROM userinfo
                                        WHERE sex = 0 AND
                                        CASE WHEN sex = ${sex}
    ...
    getOneRmMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseFloat(data[0].max_one_rm) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
    ...
    getWeightMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseFloat(data[0].max_weight) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
    ...
    getVolumeMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseFloat(data[0].max_volume) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
    ...
    getTotalSetsMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseInt(data[0].sum_total_sets) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
    ...
    getTotalVolumeMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseFloat(data[0].sum_total_volume) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
    ...
    getTotalRepsMaxRankByWorkoutId: async (workout_id, uid, sex, ageGroup, weightGroup, group_condition, period_condition) => {
        ...
        try {
            const data = await pool.queryParamSlave(query);
            return data.length != 0 ? { rank: parseInt(data[0].rank), value: parseInt(data[0].sum_total_reps) } : { rank: null, value: null };
        } catch (err) {
            if (err.errno == 1062) {
                console.log('getWorkoutRecordById ERROR: ', err.errno, err.code);
                return -1;
