...
const template = {
  getDashboardRecords: async (uid) => {
    const fields1 =
      "AVG(total_time) AS time_AVG, AVG(session_total_volume) AS volume_AVG, AVG(session_total_reps) AS reps_AVG, AVG(session_total_sets) AS sets_AVG";
    ...
    const restructure = async (result1) => {
      ...
      await asyncForEach(result1, async (rowdata) => {
        ...
        data.push({
          ...
          one_rm: Math.round(oneRmData[0].one_rm_MAX),
          total_days: rowdata.finish_num,
          total_reps: totalRepsData[0].reps_SUM,
          max_reps: maxRepsData[0].reps_MAX,
        });
      });
      return data;
    };
