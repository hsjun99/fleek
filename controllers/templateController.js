...
module.exports = {
  savetemplate: async (req, res) => {
    ...
    res
      ...
      .send(
        util.success(
          statusCode.OK,
          resMessage.WRITE_TEMPLATE_SUCCESS,
          [await Template.getUserTemplateByTemplateId(templateIdx)],
          update_time
        )
      );
  ...
  getUserTemplateWear: async (req, res) => {
    ...
    let templateData;
    // Get User Template Data
    templateData = await Template.getUserTemplateWear(uid, langCode, isPro);
    // DB Error Handling
    if (templateData == -1) {
      return res.status(statusCode.DB_ERROR).send(util.fail(statusCode.DB_ERROR, resMessage.READ_USERTEMPLATE_FAIL));
    }
  ...
  deleteUserTemplate: async (req, res) => {
    ...
    res
      .status(statusCode.OK)
      .send(util.success(statusCode.OK, resMessage.DELETE_USERTEMPLATE_SUCCESS, [{ template_id: Number(template_id) }], update_time));
