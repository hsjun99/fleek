...
module.exports = {
  checkuser: async (req, res) => {
    ...
    if (exist == -1) {
      return res
        ...
        .send(util.fail(statusCode.DB_ERROR, resMessage.SIGNUP_FAIL));
    }

    res
      .status(statusCode.OK)
      .send(
        util.success(statusCode.OK, resMessage.SIGNUP_SUCCESS, { user: exist })
      );
  ...
  signup: async (req, res) => {
    ...
    const { name, sex, age, height, weight, squat1RM, experience, is_beta } =
      req.body;
    if (
      !String(name) ||
      !String(sex) ||
    ...
    const acceptedUid = await User.postData(
      uid,
      name,
      sex,
      age,
    ...
    await asyncForEach(
      await initialUserRoutine.initRoutines(
        langCode,
        userInfo.sex,
        userInfo.ageGroup,
        userInfo.weightGroup,
        userInfo.percentage
      ),
      async (routine) => {
        await Template.postTemplateDataDetail(
          uid,
          routine.name,
          routine.detail
        );
    ...
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, resMessage.SIGNUP_SUCCESS, {
        uid: acceptedUid,
      })
    );
  },
  ...
  checkunique: async (req, res) => {
    ...
    if (unique == -1) {
      return res
        .status(statusCode.DB_ERROR)
        .send(util.fail(statusCode.DB_ERROR, resMessage.CHECK_UNIQUE_FAIL));
    }
    res.status(statusCode.OK).send(
      util.success(statusCode.OK, resMessage.CHECK_UNIQUE_SUCCESS, {
        unique: unique,
      })
    );
  },
