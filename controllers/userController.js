  ...
  unregister: async (req, res) => {
    ...
    if (result == -1) {
      return res
        ...
        .send(util.fail(statusCode.DB_ERROR, resMessage.FOLLOW_FAIL));
    }

    // Success
    res
      .status(statusCode.OK)
      .send(util.success(statusCode.OK, resMessage.FOLLOW_SUCCESS));
  },
...
*/
  ...
  getAllFollowing: async (req, res) => {
    ...
    if (following == -1) {
      return res
        .status(statusCode.DB_ERROR)
        .send(util.fail(statusCode.DB_ERROR, resMessage.READ_FOLLOWING_FAIL));
    }
    ...
    res
      .status(statusCode.OK)
      .send(
        util.success(statusCode.OK, resMessage.READ_FOLLOWING_FAIL, following)
      );
  },
  getSetting: async (req, res) => {
    ...
    res
      .status(statusCode.OK)
      .send(
        util.success(statusCode.OK, resMessage.READ_FOLLOWING_FAIL, result)
      );
  },
  updateSetting: async (req, res) => {
    ...
    if (data.lang_code == undefined || data.lang_code == null)
      data.lang_code = 1;
    ...
    res
      .status(statusCode.OK)
      .send(util.success(statusCode.OK, resMessage.READ_FOLLOWING_FAIL));
  },
  getProfile: async (req, res) => {
    ...
    profileData["session_book"] = {
      template_id:
        session_book != undefined
          ? session_book.templateUsers_templateUsers_id
          : null,
    ...
    profileData["pt_request"] = {
      ...
      request_status:
        pt_request != undefined ? pt_request.request_status : null,
    };
    ...
    res
      .status(statusCode.OK)
      .send(
        util.success(statusCode.OK, resMessage.READ_FOLLOWING_FAIL, profileData)
      );
  },
  ...
  updateName: async (req, res) => {
    ...
    if (unique == -1) {
      ...
      return res
        ...
        .send(util.fail(statusCode.CONFLICT, resMessage.READ_FOLLOWING_FAIL));
    }
  ...
  updateBodyInfo: async (req, res) => {
    ...
    const result = await User.updateBodyInfo(
      uid,
      height,
      weight,
  ...
  updateBodyInfoRecord: async (req, res) => {
    ...
    const { height, weight, skeletal_muscle_mass, body_fat_ratio } = req.body;
    const result = await User.updateBodyInfoRecord(
      uid,
  ...
  getSelfFleekData: async (req, res) => {
    ...
    [
      ...
      data.dashboard.favorite_workouts,
    ] = await Promise.all([
  ...
  getOthersFleekData: async (req, res) => {
    ...
    const { sex, age, weight, achievement, profile_url, instagram_id } =
      await getUserInfo(other_uid);
    const [ageGroup, weightGroup] = await Promise.all([
      await ageGroupClassifier(age),
      await weightGroupClassifier(weight, sex),
    ]);
    ...
    const getOthersCustomWorkout = async () => {
      const result = await Workout.getOthersCustomWorkouts(
        other_uid,
        sex,
        ageGroup,
        weightGroup
    ...
    [data.template, data.calendar_data, data.extra_custom_workout_table] =
      await Promise.all([
  ...
  updateUserWorkoutMemo: async (req, res) => {
    ...
    const result = await UserWorkoutMemo.updateMemo(
      uid,
      userWorkoutMemo_id,
      content
    );
